...
import com.suite.loaders.ObjectDefinition639;
...
public class VarBitGUI extends JFrame {
	...
	private void itemListMouseReleased(MouseEvent e) {
		VarBitDefinition item = new VarBitDefinition(SuiteUI.cache(),
	...
	private void itemListMouseReleased1(MouseEvent e) {
		...
		if (item != null) {
			switch (list.getSelectedIndex()) {
				case 0:
					if (item.baseVar != -1)
						lblValue.setText("baseVar: "+(item.baseVar));
					else
						lblValue.setText("object_model_ids: -1");
				case 1:
					if (item.endBit != -1)
						label.setText("endBit: "+item.endBit);
					else
						label.setText("endBit: null");
					break;
				case 2:
					if (item.endBit != -1)
						lblValue.setText("endBit: "+item.endBit);
					else
						lblValue.setText("endBit: -1");
					break;
				/*case 2:
					if (item.name != "null")
	...
	private void objectPackMouseReleased(MouseEvent e) {
		VarBitDefinition.objectPack(SuiteUI.cache());
